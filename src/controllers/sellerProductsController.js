...
function validateAndMapProductPayload(data, { forUpdate = false } = {}) {
  ...
  if (!forUpdate) {
    ...
    // Bar_code is optional on create — backend will generate if missing
    if (data.Bar_code !== undefined && typeof data.Bar_code !== 'string') fieldErrors.Bar_code = 'Bar_code must be a string if provided';
  }
